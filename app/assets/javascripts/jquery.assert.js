...
;(function( $, undefined ){
  ...
  $.assert = function (options){
     ...
     */
    $.fn.assertFound = function(expectedSize, selector) {
      ...
      if(expectedSize === undefined && this.length === 0){
        message = "Assertion failed, expected to find elements using selector '" + selector + "', but found none."
        _log(message)
        throw message;
      }
      ...
      if (expectedSize !== undefined && this.length !== expectedSize) {
        message = "Assertion failed, expected size: " + expectedSize + ", actual size: " + this.length + ", selector: '" + selector + "'.";
        _log(message)
        throw message;
      }
     ...
     */
    $.fn.assertOne = function(selector) { return this.assertFound(1, selector); };

    /**
     * Convenience function for '.assertFound()'
     */
    $.fn.assertNotEmpty = function(selector) {
      return this.assertFound(undefined, selector);
    };
    ...
    function extendedJQuery(selector, context, expectedSize) {
      ...
      if(typeof arguments[arguments.length-1] === 'number'){
        ...
        result.assertFound(expectedSize, selector);
        return result;
      }
...
window.$ao = function(selector) {
  return $(selector).assertOne(selector);
};

window.$af = function(selector, count) {
  return $(selector).assertFound(count, selector);
};
